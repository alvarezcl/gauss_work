...
for i in xrange(0,loops):
    ...
    meanp = 0; sigmap = start*sigma*(1+i); varp = sigmap**2; N_s = N*Ars*sigmap
...
p2 = plt.figure(2); plt.xlabel(r'$\sigma_r=\frac{\sigma_s}{\sigma_p}$',fontsize=font); plt.ylabel('$Fit\/\sigma_{est}$',fontsize=font)
...
plt.suptitle(plotutils.info_gaussian_Acp('Primary',Arp,mean,sigma)+ '\n' +
             'Secondary: $A_{cs}=%.2f$, $\mu=%.2f$, $\sigma\/\epsilon[0.1,%.1f]$'%(Ars,meanp,loops*start),fontsize=font_title)
plt.show()

p3 = plt.figure(3); plt.xlabel(r'$\sigma_r=\frac{\sigma_s}{\sigma_p}$',fontsize=font); plt.ylabel('$Fit\/\mu_{est}$',fontsize=font)
...
p4 = plt.figure(4); plt.xlabel(r'$\sigma_r=\frac{\sigma_s}{\sigma_p}$',fontsize=font); plt.ylabel('$Fit\/A_{est}$',fontsize=font)
